...
"""PyTorch MPT model implementing Eigen Attention."""
...
class MptEigenAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.Wqkv_low = nn.Linear(self.hidden_size, (2*rank_kq + rank_v), bias=False)
        # self.vproj_up = nn.Linear(rank_v // self.n_heads, self.head_dim, bias = False)

        # self.out_proj = nn.Linear(self.hidden_size, self.hidden_size, bias=False)
        self.out_proj_up = torch.nn.Linear(self.rank_v, self.hidden_size, bias = False)
    ...
    def forward(
        ...
        mixed_qkv = self.Wqkv_low(hidden_states)
        ...
        attn_output = self.out_proj_up(context_states)

        return attn_output, attn_weights, past_key_value



class MptBlock_EigenAttn(nn.Module):
    def __init__(self, low_rank_config, config: MptConfig):
        super().__init__()
        ...
        self.attn = MptEigenAttention(config = config, rank_kq = int(low_rank_config[0]), rank_v = int(low_rank_config[1]))
...
class MptModel_EigenAttn(MptPreTrainedModel):
    def __init__(self, low_rank_config, config: MptConfig):
        ...
        # Transformer blocks
        self.blocks = nn.ModuleList([MptBlock_EigenAttn(config = config, low_rank_config = low_rank_config[i]) for i in range(config.n_layers)])
...
class MptForCausalLM_EigenAttn(MptPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: MptConfig, **kwargs):
        low_rank_config = kwargs['low_rank_config']
        super().__init__(low_rank_config = low_rank_config, config = config)
        self.transformer = MptModel_EigenAttn(config = config, low_rank_config=low_rank_config)
...
class MptForSequenceClassification_EigenAttn(MptPreTrainedModel):
    def __init__(self, low_rank_config, config: MptConfig):
        super().__init__(config)
...
class MptForTokenClassification_EigenAttn(MptPreTrainedModel):
    def __init__(self, low_rank_config , config: MptConfig):
        super().__init__(config)
        self.num_labels = config.num_labels
