...
def get_alpaca(nsamples, seed, seqlen, model):
    print("get alpaca")
    ds = load_dataset('tatsu-lab/alpaca')
    ds = ds.remove_columns(['input', 'output', 'instruction'])
    traindata = ds["train"]
    if 'mpt' in model.lower():
...
def get_loaders(
    ...
    if 'mix' in name:
        ...
        # ptb_train,ptb_val=get_ptb(nsamples//2, seed, seqlen, model)
        c4_train,c4_val=get_c4(nsamples//2, seed, seqlen, model)
        # alpaca_train = get_alpaca(nsamples//3, seed, seqlen, model)
        train=wiki_train+c4_train
        val=None
        return train,val
    if 'alpaca' in name:
        train = get_alpaca(nsamples, seed, seqlen, model)
        return train, None
